...
(the `BmpCodec`, `PngCodec`, `TiffCodec`, `JpegCodec`, and `SvgCodec` units).
...
_SvgCodec Unit Verification Design_ under `codecs/`), together with the system-level round-trip
(or, for the decode-only `SvgCodec`, load-only) integration tests in `CanvasNetTests.cs` that
exercise each codec end-to-end against a `Surface`. No separate subsystem-level tests exist; the
subsystem-level requirements reuse the corresponding unit and system-integration tests as
...
- **Mocking**: None required; `BmpCodec`, `PngCodec`, `TiffCodec`, and `JpegCodec`'s only
  dependency is the in-house `Canvas` subsystem's `Surface` unit; `SvgCodec` additionally
